...
def generate_random_user():
    return {
        ...
        "cash": random.randint(1000, 10000)
    }
...
def insert_data_through_api(base_url, database_name, table_name, num_records):
    ...
    for _ in range(num_records):
        ...
        response = requests.post(f"{base_url}/{database_name}/{table_name}", json={"operation": "CREATE", "new_record": user_data})
...
if __name__ == "__main__":
    ...
    database_name = 'my_db'
    table_name = 'users'
    num_records = 10  # Defina o número de registros que deseja inserir
